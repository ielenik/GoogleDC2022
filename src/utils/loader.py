...
def myLoadRinexIndexed(path, sat_registry = {}):
    if os.path.exists(path+'.all.pkl'):
        with open(path+'.all.pkl', 'rb') as f:
    ...
    for val in tqdm(base_all):
        if val[0] != 'C' and val[0] != 'L':
            continue
        if val[1] != '1' and val[1] != '5' and val[1] != '2':
            continue
        ...
        try:
            ...
            for ob in base:
                ...
                for sat in ob:
                    if np.isnan(float(sat)) or float(sat) == 0:
                        continue
    ...
    result = (sat_distances,base_all.position,sat_registry)
    with open(path+'.all.pkl', 'wb') as f:
