...
def calc_track_speed(trip_id):
    ...
    coords_start = np.median(baselines[0:15], axis = 0)
    ...
    baselines, gfit = calc_initial_speed_guess(sat_deltarange,sat_deltaweights,-sat_deltaspeed, 1/sat_deltaspeeduncert, sat_types, sat_directions, gt_np,acsvalues,acstimes,gyrvalues,gyrtimes,utcTimeMillis,baseline_stable.copy())
    ...
    # or_init[0] = [0.,0.,0.]
    # imu_gyr_cum = imu_model.gyr[::10,1]
    # or_init[1:,2] = (imu_gyr_cum[2:-2] / imu_model.angle_scale).numpy()
    # or_init = or_init.astype(np.float32)
    or_init[0,2] = gfit[0]/imu_model.angle_scale
    imu_med = np.median(imu_model.gyr.numpy(), axis = 0)
    imu_model.gyro_bias = tf.Variable([[-imu_med[0],gfit[1]/20000,-imu_med[2]]], name = 'gyro_bias', trainable=True, dtype = tf.float32)
    orients = tf.Variable(or_init,trainable=True,dtype=tf.float32)
    or_init[1:,2] = imu_model.get_angles([0,orients], False)[0].numpy()[:,2]/imu_model.angle_scale

    med = np.median(np.cumsum(or_init, axis = 0), axis = 0)[2]*imu_model.angle_scale
    shift = (med/(PI*2)).astype(np.int32)
    or_init[0,2] -= shift*2*PI/imu_model.angle_scale
    med -= shift*2*PI
    shift = (med/PI).astype(np.int32)
    or_init[0,2] -= shift*2*PI/imu_model.angle_scale
    med -= shift*2*PI
    orients = tf.Variable(or_init,trainable=True,dtype=tf.float32)

    # plt.clf()
    # plt.plot( np.arange(len(or_init)), np.cumsum(or_init, axis = 0)*imu_model.angle_scale)
    ...
    np_firstlast = np.ones(len(times_dif))
    ...
    np_firstlast[:len_frst]  = 0
    np_firstlast[-5:] = 0
    firstlast_epoch = tf.Variable(np_firstlast.reshape((-1,1)),trainable=False,dtype=tf.float32)
    ...
    def minimize_speederror(useimuloss, trainorient, trainspeed, rotate_gyro, optimizer):
        ...
        for _ in range(4):
            with tf.GradientTape(persistent=True) as tape:
                ...
                sp = speeds*1000./times_dif
                acs = sp[1:]-sp[:-1]
                acs = tf.concat([acs,[[0.,0.,0.,]]], axis = 0)
                speeds_loss_glob = tf.square(norm(acs))#norm(acs)*10+
                ...
                acs_loss, quat_loss, speed_loss, g, stable_poses = imu_model([speeds,orients,times_dif,rotate_gyro])
                poses = tf.cumsum(speeds, axis = 0)
                poses = tf.concat([[[0.,0.,0.]],poses], axis = 0)

                angles_cum  = imu_model.get_abs_orientation(orients)
                fwd = tf.stack([tf.math.cos(angles_cum[:,2]),tf.math.sin(angles_cum[:,2]),tf.zeros_like(angles_cum[:,2])], axis = 1)


                #psevdo_loss = psevdo_model([tf.concat([[[0.,0.,0.]],speeds], axis = 0), poses - baselines, times])
                psevdo_loss = psevdo_model([tf.concat([[[0.,0.,0.]],fwd], axis = 0), poses - baseline_stable, times])
                imu_loss = quat_loss + speed_loss + acs_loss
                imu_loss = tf.concat([[0.],imu_loss], axis = 0)
                imu_loss = imu_loss*np_firstlast
                dir_loss = quat_loss + speed_loss + acs_loss
                dir_loss = tf.concat([[0.],dir_loss], axis = 0)
                if not useimuloss:
                    total_loss = phase_loss #+ speeds_loss_glob/10

                    #total_loss = speeds_loss_glob/3 + speeds_loss_Z*10 + phase_loss*10 + dopler_loss*10# + psevdo_loss*400  #+ dir_loss*10   
                    #total_loss = psevdo_loss
                else:   
                    total_loss = phase_loss + imu_loss/100 + dopler_loss/100 + speeds_loss_Z/10 + psevdo_loss
            ...
            speeds.assign(speeds*firstlast_epoch)
            del tape


        return  total_loss, phase_loss, dopler_loss, acs_loss, quat_loss, speed_loss, psevdo_loss, speeds_loss_Z, g, psevdo_model.get_poses([tf.concat([[fwd[0]],fwd],axis=0),poses, times]), stable_poses
    ...
    for step in range(0, 64):
        ...
        if step == 0:
            ...
            optimizer = tf.keras.optimizers.Adam(learning_rate=lr)#, epsilon= 0.0001)
            for i in range(1):
                total_loss, phase_loss, dopler_loss, acs_loss, quat_loss, speed_loss,psevdo_loss, speeds_loss_glob, g, poses, stable_poses  = minimize_speederror(False, True, False, False, optimizer)
            ...
            #optimizer = tf.keras.optimizers.SGD(learning_rate=lr, momentum=0.9)
            optimizer = tf.keras.optimizers.Adam(learning_rate=lr)#, epsilon= 0.0001)
            train_step = tf.function(minimize_speederror).get_concrete_function(False, False, trainspeed, True, optimizer)
        ...
        #     train_step = tf.function(minimize_speederror).get_concrete_function(True, True, trainspeed, True, optimizer)
            ...
            train_step = tf.function(minimize_speederror).get_concrete_function(True, True, True, True, optimizer)


        if step != 0:
            for i in range(128):
                total_loss, phase_loss, dopler_loss, acs_loss, quat_loss, speed_loss,psevdo_loss, speeds_loss_glob, g, poses, stable_poses  = train_step()
        ...
        print( "Training loss at step %d: %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f  lr %.4f er %.4f ser %.4f" % (step, 
            ...
            #,psevdo_model.shift1.numpy()
            ,psevdo_model.shift0.numpy()
            )#, end='\r')
        ...
        if True:#step%16 == 0:
            ...
            try:
                ...
                plt.plot( np.arange(len(speeds_init_gt)), speeds_init_pred[:,0]*1000/timedif)
                plt.plot( np.arange(len(speeds_init_gt)), speeds_init_gt[:,0]*1000/timedif)
                plt.plot( np.arange(len(speeds_init_gt)), speeds_init_pred[:,1]*1000/timedif)
                plt.plot( np.arange(len(speeds_init_gt)), speeds_init_gt[:,1]*1000/timedif)
                ...
                plt.legend(['px', 'tx', 'py', 'ty', 'acs','quat','speed', 'speed dif'])
                #plt.legend(['dx', 'dy', 'phase', 'dopler', 'acs','quat','speed', 'psevdo'])
                #plt.show()
                save_fig(image_path+'spd\\speed_diff'+str(step).zfill(3)+'.png')
            except:
                _ = 0
            ...
            quats_np  = imu_model.get_abs_orientation(orients).numpy()
            ga = quats_np[:,2]
            ...
            ga = quats_np[:,2]
            ga = ga - (ga/(2*PI)).astype(np.int32)*2*PI
            ga = ga - (ga/(PI)).astype(np.int32)*2*PI
            ...
            kml = KMLWriter(image_path+ "kml\\kml_"+str(step).zfill(3)+".kml", "predicted")
    ...
    pred_pos = poses.numpy()
    wgs_poses = ecef2geodetic(local_transform_inv(pred_pos))
    with open(f'{GOOGLE_DATA_ROOT}{trip_id}'+'/result.pkl', 'wb') as f:
