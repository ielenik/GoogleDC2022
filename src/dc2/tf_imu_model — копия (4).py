...
class RigidModelImu(tf.keras.layers.Layer):
    ...
    def get_global_acs(self, angles_cum):
        orientation = tf.linalg.l2_normalize(self.ort, axis = 0)
        
        acsl = self.callconv_acel(self.acs)
        acsl = tf.linalg.matvec(orientation, acsl)
        mat = mat_from_euler(angles_cum)
        acsl = tf.linalg.matvec(mat, acsl)

        return acsl

    def get_local_gyr(self, angles_cum, gyrpred):
        orientation = tf.linalg.l2_normalize(self.ort, axis = 0)
        
        gyrl_tr = self.callconv_gyro(self.gyr)
        gyrl_tr = tf.linalg.matvec(orientation, gyrl_tr)
        mat = rot_mat_from_euler(angles_cum)
        gyrl_pr = tf.linalg.matvec(mat, gyrpred)

        return gyrl_tr, gyrl_pr

    def call(self, inputs):
        speed, angles_cum, times_dif = inputs
        speed = speed*1000/times_dif
        angles_cum_mid = (angles_cum[1:]+angles_cum[:-1])/2
        acsi = self.get_global_acs(angles_cum_mid)

        acs_pred = speed[1:] - speed[:-1]
        acsi_norm = tf.linalg.l2_normalize(acsi*self.g_scale, axis = -1)

        acsi = acsi*self.g_scale  + self.down*self.g
        speed_loc = tf.cumsum(acsi, axis = 0)
        speed_loc = tf.concat([[[0,0,0]],speed_loc], axis = 0)
        acsi_norm = tf.concat([[[0,0,1]],acsi_norm], axis = 0)
        ...
        pred_gyr = angles_cum[1:]-angles_cum[:-1]
        gyrl_tr, gyrl_pr = self.get_local_gyr(angles_cum_mid,pred_gyr)
        quat_loss = tf.reduce_sum(tf.square(gyrl_tr-gyrl_pr), axis = -1)/(1e-6+my_norm(gyrl_tr)*100) + 100*tf.reduce_mean(tf.square(self.gyro_bias[1:]-self.gyro_bias[:-1]))
        quat_loss = tf.concat([[0],quat_loss], axis = 0) + tf.square(1 - acsi_norm[:,2])
    ...
    def get_acses(self, inputs):
        ...
        angles_cum_mid = (angles_cum[1:]+angles_cum[:-1])/2
        acsl = self.get_global_acs(angles_cum_mid)
